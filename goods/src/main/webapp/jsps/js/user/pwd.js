// 换一张
function _hyz() {
	$("#vCode").attr("src", "/goods/user/getVerify.action?" + new Date());
...
function validateLoginpass() {
	...
	if (!value) { // 非空校验
		...
		bool = false;
	} else if (value.length < 2 || value.length > 12) { //长度校验
		$("#loginpassError").css("display", "");
		$("#loginpassError").text("密码长度必须在2 ~ 12之间！");
		bool = false;
	} else { // 验证原错误是否正确
		$.ajax({
			cache : false,
			...
			url : "/goods/user/verifyPwd.action",
...
function validateNewpass() {
	...
	if (!value) { // 非空校验
		...
		$("#newpassError").css("display", "");
		$("#newpassError").text("新密码长度必须在2 ~ 12之间！");
		bool = false;
	}
	return bool;
}
